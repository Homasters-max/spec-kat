# Spec_v21 — Phase 21: Mutation Governance Runtime (OSML)

Status: Draft
Baseline: Spec_v20_TemporalNavigation.md

---

## 0. Goal

Phase 20 дала агенту **временную навигацию** — он умеет спрашивать "что изменилось и почему".
...
Phase 21 вводит **Out-of-Scope Mutation Ledger (OSML)** — третий governance layer:
...
- **BC-21-0: Domain Events** — `OutOfScopeMutationFlagged` + `OutOfScopeChangeRecorded` (events.py)
- **BC-21-1: SDDState Extension** — `pending_osml_mutations` + reducer logic (reducer.py)
- **BC-21-2: yaml_state serialization** — секция `osml:` в State_index.yaml (yaml_state.py)
- **BC-21-3: flag-mutation command** — Layer 1 Detection (flag_mutation.py)
- **BC-21-4: record-change command** — Layer 2 Resolution (record_change.py)
- **BC-21-5: OSMLGuard** — Layer 3 Enforcement (osml_guard.py)
- **BC-21-6: scope.py evolution** — NORM-SCOPE-005 + `scope_status` field
- **BC-21-7: Registry + CLI** — регистрация, Click-команды, cli.schema.yaml
- **BC-21-8: Norm Catalog** — NORM-AUDIT-001, NORM-SCOPE-005
- **BC-21-9: CLAUDE.md invariants** — I-AUDIT-1, I-AUDIT-2(deferred)
- **BC-21-10: Tests** — unit + integration

### Out of Scope

- **I-AUDIT-2 enforcement** — diff-validation против git (требует git integration, Phase 22)
...
### BC-21-1: SDDState Extension + Reducer
...
### BC-21-2: yaml_state Serialization
...
class FlagMutationHandler(CommandHandlerBase):
    ...
    def handle(self, cmd: FlagMutationCommand) -> list[DomainEvent]:
        ...
        return [OutOfScopeMutationFlaggedEvent(
            ...
            event_source="runtime",          # MUST be runtime (not meta) — BC-21-0
            caused_by_meta_seq=None,
...
### BC-21-6: scope.py Evolution (NORM-SCOPE-005)
...
| I-AUDIT-2          | (DEFERRED Ph22) recorded changes MUST correspond to actual git diffs  |
...
| I-SCOPE-2          | (DEFERRED Ph22) TaskOutputs SHOULD be loaded from TaskSet, not LLM CLI |
```
...
**C-1 assertion (I-ST-10)** после Phase 21:
...
Исторические события (до Phase 21) не затронуты.
...
| I-AUDIT-2 | (DEFERRED) recorded changes MUST correspond to actual git diffs | 22 | Phase 22 |
...
| I-OSML-5 | `flag-mutation` and `record-change` MUST refer to the same `file_path` as the actual write. Mechanical enforcement deferred to Phase 22 (I-AUDIT-2 + git diff). Phase 21: invariant documented, not enforced. | 22 | Phase 22 git diff integration |
...
| I-SCOPE-2 | TaskOutputs SHOULD be loaded from TaskSet, not provided by LLM. DEFERRED Phase 22: CLI `--outputs` remains for backward compat; Phase 22 adds auto-load from TaskSet with LLM override only for debug. | 22 | Phase 22 |
...
**Pre:** Phase 21 ACTIVE, T-042 ACTIVE
...
**Pre:** Phase 21 COMPLETE с несколькими resolved mutations
...
| Spec_v20 Temporal | BC-21 builds on | REDUCER_VERSION bump is additive |

### Architecture Position

```
                     ┌─────────────────────────────────────────┐
                     │  Tri-Layer Governance (Phase 21)         │
                     │                                          │
...
| Item | Owner |
|------|-------|
| I-AUDIT-2: diff validation against git | Phase 22 |
| I-OSML-5 enforcement: file_path vs actual write correspondence | Phase 22 (git diff) |
| I-SCOPE-2: auto-load TaskOutputs from TaskSet (not CLI) | Phase 22 |
| Severity-based guard escalation (high severity → block) | Phase 22+ |
| OSML phase summary (total/resolved/unresolved counts) | Phase 22 |
| Filesystem write hooks (auto-intercept) | никогда |
| Phase 17-20 test deletion | никогда (CEP-3) |
| Distributed/concurrent OSML scenarios | Phase 22 |
...
```
           ...
           │  + Graph (edges)     │  ← связи + priority (Phase 19)
           └──────────┬───────────┘
                      ↓
      ┌───────────────┴────────────────┐
      ↓                                ↓
...
Phase 20                         Phase 20
                                 +
                                 OutOfScopeMutationFlagged   ← Phase 21
                                 OutOfScopeChangeRecorded    ← Phase 21
```
...
| event_source="meta" silently ignored by reducer | event_source="runtime" обязателен (см. BC-21-0) |
