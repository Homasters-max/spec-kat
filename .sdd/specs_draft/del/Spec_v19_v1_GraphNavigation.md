# Spec_v19_v1 — Phase 19: Graph Navigation (GN)

Status: Draft — ACTIVE (запускается первой; требует только Phase 18 + Postgres DB via $SDD_DSN)
Note: Spec_v36_GraphNavigation.md = Post-Phase-32 upgrade (унифицирует $SDD_DATABASE_URL + p_sdd схему). Phase 19 и Phase 36 — разные реализации одного слоя для разных стадий инфраструктуры.
Baseline: Spec_v18_SpatialIndex.md
Supersedes: Spec_v19_GraphNavigation.md (архивируется в del/)
...
1. **Postgres backend** — граф живёт в схеме `graph` единой Postgres БД. DuckDB
   используется только для EventLog (`public` схема) до Phase 30 (Postgres-миграция).
...
- **BC-19-0: Postgres Graph Schema** — `graph.spatial_nodes`, `graph.spatial_edges`,
  `graph.node_tags` (schema.py)
- **BC-19-1: SpatialIndex API Extension** — `iter_files()`, `iter_terms()`,
  `iter_invariants()`, `iter_tasks()`, `typed_registry()`, `read_content()`, `content_map`
  добавляются в `SpatialIndex`; `frozen=True` (I-SI-SNAPSHOT-1)
- **BC-19-2: IndexBuilder Extension** — INVARIANT-узлы мигрируют с CLAUDE.md →
  `invariants.yaml`; IndexBuilder остаётся единственным filesystem reader
- **BC-19-3: Graph Loader** — FS-free преобразователь: `build_graph(index, conn)`,
  AST-сканы через `iter_files()`, config-рёбра через `iter_terms/invariants/tasks()`
  (loader.py)
- **BC-19-4: GraphQuerier** — `get_node()`, `search_nodes()`, `neighbors()` через
  psycopg + `WITH RECURSIVE` CTE (querier.py)
- **BC-19-5: TERM Integration** — typed `means`-рёбра; I-DDD-1 (typed reference check)
- **BC-19-6: sdd resolve** — unified DDD entrypoint: search → resolve → neighbors
  (nav_resolve.py), Postgres-native fuzzy
- **BC-19-7: nav-neighbors, nav-invariant** — дополнительные CLI-команды
- **BC-19-8: Tests** — unit + integration
- **BC-19-9: Backend registration** — nav-get/nav-rebuild получают `--backend postgres`
- **BC-19-10: invariants.yaml + sdd nav-export** — machine-readable инварианты;
  I-INV-EXPORT-1

### Out of Scope

- `observed_in` edges, `eventlog-projector`, `nav-drift` — Phase N (после Postgres
  EventLog migration, Phase 32+). Запрещено смешивать EventLog и graph layer в Phase 19.
...
- `graph.*` — граф (Phase 19)
- `public.*` — EventLog (DuckDB до Phase 30)
- Нет FK между схемами — граф является производной проекцией (I-1)

### BC-19-1: SpatialIndex API Extension
...
### BC-19-2: IndexBuilder Extension
...
def _build_invariant_nodes(self) -> list[SpatialNode]:
    """
    Reads .sdd/config/invariants.yaml (SSOT, Phase 19+).
...
class GraphQuerier:
    ...
    def __init__(self, dsn: str):
        self._dsn = dsn  # I-DB-1: explicit non-empty str
...
выполняется внутри `GraphLoader.build_graph()` при построении графа (BC-19-3).
`GraphQuerier` — чистый read-интерфейс без validation concerns.

### BC-19-5: TERM Integration (I-DDD-1)
...
### BC-19-6: sdd resolve — Unified DDD Entrypoint
...
### BC-19-7: nav-neighbors, nav-invariant
...
### BC-19-10: invariants.yaml + sdd nav-export
...
### New Invariants — Phase 19

| ID | Statement | Phase | Verification |
|----|-----------|-------|-------------|
| I-SI-SNAPSHOT-1 | `SpatialIndex` = иммутабельный snapshot (frozen=True). Инвалидация — исключительно внешняя (staleness.py / nav-rebuild). Внутреннего cache, lazy-load, file I/O внутри `SpatialIndex` — нет. | 19 | `test_index_api.py` (frozen check) |
| I-INDEX-FILE-COVERAGE-1 | All Python files in `src/sdd/` relevant for graph construction MUST be represented as FILE nodes in SpatialIndex | 19 | `test_index_api.py` |
| I-GRAPH-FS-ISOLATION-1 | GraphLoader MUST NOT access filesystem directly. All file content via `SpatialIndex.read_content(node)`. No carve-out. | 19 | `test_loader.py` (patch builtins.open + FakeIndex with FILE nodes) |
| I-GRAPH-1 | Every edge derives from static AST or explicit config (SpatialIndex API); no heuristics, no causal inference | 19 | `test_loader.py` |
| I-GRAPH-2 | `edge_source VARCHAR NOT NULL` — детерминированный источник каждого ребра | 19 | SQL check + `test_loader.py` |
| I-GRAPH-3 | Every INVARIANT node has ≥1 outgoing `verified_by` or `introduced_in` edge | 19 | SQL check + `test_loader.py` |
| I-GRAPH-EMITS-1 | `emits` edge requires all 4 conditions: handle() + AST return + DomainEvent registry + handler binding | 19 | `test_loader.py` |
| I-GRAPH-PRIORITY-1 | `nav-neighbors` sorted by `hop ASC, priority DESC, dst ASC` | 19 | `test_querier.py`, `test_nav_neighbors.py` |
| I-DDD-1 | ∀ TERM.link → typed_registry.has(namespace, id); broken typed ref = hard violation | 19 | `GraphLoader.build_graph()` + `test_nav_postgres.py` |
| I-DDD-2 | `sdd resolve` MUST return `must_not_guess: true` on not_found | 19 | `test_nav_resolve.py` |
| I-TERM-REGISTRY-SOURCE-OF-TRUTH-1 | registry.py = SSOT для COMMAND; glossary.yaml = semantic projection only | 19 | `test_loader.py` |
| I-INV-EXPORT-1 | После изменения invariants.yaml LLM MUST вызвать `sdd nav-export --invariants` | 19 | protocol (CI diff check) |
...
- Postgres DB доступна; `$SDD_DSN` установлен
...
- `.sdd/config/invariants.yaml` создан с Phase 18 + Phase 19 инвариантами
...
### UC-19-1: nav-rebuild запускает pipeline

**Actor:** разработчик / LLM  
**Trigger:** `sdd nav-rebuild --backend postgres`  
**Steps:**
1. `IndexBuilder(root).build()` — единственный fs-проход, строит все узлы
2. `SpatialIndex` готов с полным API
3. `GraphLoader().build_graph(index, conn)` — FS-free, строит рёбра из API
4. Postgres upsert: nodes + edges
5. I-DDD-1 check внутри `build_graph`  
**Post:** граф актуален; pipeline выполнен за один fs-проход

### UC-19-2: Agent Starts Navigation via sdd resolve
...
| Item | Owner |
|------|-------|
| `observed_in` edges, eventlog-projector, nav-drift | Phase 32+ |
| Causal graph | Запрещено навсегда (I-GRAPH-1) |
| Temporal navigation | Phase 20 |
| TaskCheckpoint events | Phase 20 |
| NORM-NAV-001 в norm_catalog | Phase 20 |
| ML ranking, embedding, shortest path | Никогда |
| Graph visualization, Apache AGE | Никогда |
| DuckDB EventLog migration | Phase 30 |

---

## 10. Dependencies & Impact

### Upstream (Phase 19 requires)

| Dependency | Required from |
|-----------|---------------|
| `spatial_index.json` актуален | Phase 18 COMPLETE |
| `nodes.py` с `TermNode` и `SpatialEdge` | Phase 18 M1 |
| `glossary.yaml` с typed links | Phase 18 (расширяется в Phase 19) |
| Postgres DB; `$SDD_DSN` env var | Infra |

### Downstream (Phase 20 requires from Phase 19)

| What | Where used |
|------|-----------|
| `SpatialIndex` iter_* API | Phase 20 temporal queries |
| `graph.spatial_nodes/edges` schema | Phase 20 temporal extensions |
| `invariants.yaml` format | Phase 20 new invariants |
| `GraphQuerier` interface | Phase 20 temporal extensions |
| `nav-export` infrastructure | Phase 20+ additions |

### Impact на Spec_v30 (DocFixes)

BC-30-5 упрощается: `dev-cycle-map.md` содержит только ссылки `→ см. CLAUDE.md §INV`.
CLAUDE.md §INV = generated view из `invariants.yaml` (Phase 19).
