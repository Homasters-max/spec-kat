# Spec_v20 — Phase 20: Temporal Navigation (TN)

Status: Draft
Baseline: Spec_v19_GraphNavigation.md

---

## 0. Goal

Phase 19 дала агенту граф связей — он умеет перемещаться по структуре системы.
Но граф статичен: агент не знает **что изменилось** и **почему это изменилось**.

Phase 20 вводит **Temporal Navigation** — объединение пространства (граф) и времени (git + EventLog):
...
- **BC-20-0: TaskCheckpoint event** — новый domain event (events.py)
- **BC-20-1: ContentAddressableStore + git_bridge** — абстракция над git (git_bridge.py)
- **BC-20-2: changed_since** — детерминированный temporal query (changed_since.py)
- **BC-20-3: update_state changes** — `sdd complete` создаёт git commit; I-HANDLER-PURE-1 сохранён
- **BC-20-4: validate_invariants extension** — проверка I-TEMP-1
- **BC-20-5: CLI commands** — `sdd nav-changed-since`, `sdd nav-task-commits`
- **BC-20-6: NORM-NAV-001** — в `.sdd/norms/norm_catalog.yaml`
- **BC-20-7: Tests** — unit + integration
...
### BC-20-1: ContentAddressableStore + git_bridge
...
### BC-20-3: update_state.py Changes
...
### BC-20-4: validate_invariants Extension
...
Исторические `TaskImplemented` события (до Phase 20) имеют `commit_sha = null`.
...
**Pre:** Phase 20 ACTIVE, T-2001 COMPLETE (TaskImplemented с commit_sha)
**Steps:**
...
**Pre:** Phase 20 ACTIVE, есть незакоммиченные изменения
**Steps:**
...
**Post:** каждая завершённая задача Phase 20+ имеет git boundary

### UC-20-3: Checkpoint During Long Task

**Actor:** LLM-агент
**Trigger:** `sdd checkpoint T-2004 --message "add git_bridge skeleton"`
**Pre:** Phase 20 ACTIVE, T-2004 ACTIVE, есть изменения
**Steps:**
...
**Pre:** Phase 20 ACTIVE, индекс не изменился
**Steps:**
...
# I-TEMP-1: commit_sha в TaskImplemented (Phase 20 задачи)
sdd validate-invariants --check I-TEMP-1 --phase 20    # exit 0 PASS
...
4. `sdd nav-task-commits --phase 20` — нет null `commit_sha` для Phase 20 задач
...
Инварианты I-TEMP-2..3 (Phase 20) управляют шагом 6.
